...
bandwidth=[15 37];
[b,a]=butter(4,[15 37]./(Fs/2),'bandpass');
...
for fold_num=1:length(subject)
    ...
    day1acc_high_tmp(fold_num,:,:)=sub_stimacc(1:5,:);day2acc_high_tmp(fold_num,:,:)=sub_stimacc(6:10,:);
end
% re-arrange stim order
day1acc_high=zeros(14,5,10);day2acc_high=zeros(14,5,10);
for nbtrial=1:10
    stimnb=stim_order(nbtrial);
    day1acc_high(:,:, stimnb)=day1acc_high_tmp(:,:,nbtrial);
    day2acc_high(:,:, stimnb)=day2acc_high_tmp(:,:,nbtrial);
end
save('day1acc_high','day1acc_high')
save('day2acc_high','day2acc_high')