function varargout=sigstar(groups,stats,nosort,color,nbplot,nbexp)
...
y=ylim;
if nbplot==3 && nbexp==1
    yd=myRange(y)*0.28; %!!! changed from 0.1 to 0.05
else
...
function H=makeSignificanceBar(x,y,p,color)
...
H(1)=plot(x(:),y,'-','LineWidth',1.5,'Tag','sigstar_bar','Color',color);
...
H(2)=text(mean(x(:)),starY,stars,...
    ...
    'Tag','sigstar_stars','Color',color,'FontSize',14);
