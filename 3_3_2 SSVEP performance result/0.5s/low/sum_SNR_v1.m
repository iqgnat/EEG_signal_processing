...
for nbchannel=1:16
    for nbstim=1:10
        [h,p,~,~] = ttest(squeeze(day_snr(nbchannel,nbstim,1,:)),squeeze(day_snr(nbchannel,nbstim,2,:)),0.01,'left');
        sav_h(nbchannel,nbstim)=h;
    end
end
sav_h
% summray: 
% no correlation for 0.5s low SNR change in two day
